 ...
 *
 * The file `tests/example_sig.c` contains two examples on using the OQS_SIG
 * API.
 *
 ...
 * sizes indicated using preprocessor macros.  Since algorithms can be disabled
 * at compile-time, the programmer should wrap the code in \#ifdefs.
 *
 ...
 * malloc'ed by the programmer, with sizes indicated using the corresponding
 * length member of the OQS_SIG object in question.  Since algorithms can be
 * disabled at compile-time, the programmer should check that the OQS_SIG object
 * is not `NULL`.
 *
...
/** Algorithm identifier for KAZ-SIGN NIST Level 1 */
#define OQS_SIG_alg_kaz_sign_1 "KAZ-SIGN-1"
/** Algorithm identifier for KAZ-SIGN NIST Level 3 */
#define OQS_SIG_alg_kaz_sign_3 "KAZ-SIGN-3"
/** Algorithm identifier for KAZ-SIGN NIST Level 5 */
#define OQS_SIG_alg_kaz_sign_5 "KAZ-SIGN-5"
...
/**
 * Returns identifiers for available signature schemes in liboqs.  Used with
 * OQS_SIG_new.
 *
 * Note that algorithm identifiers are present in this list even when the
 * algorithm is disabled at compile time.
 *
 * @param[in] i Index of the algorithm identifier to return, 0 <= i <
 * OQS_SIG_algs_length
...
/**
 * Returns the number of signature mechanisms in liboqs.  They can be enumerated
 * with OQS_SIG_alg_identifier.
 *
 ...
 *
 * @param[in] method_name Name of the desired algorithm; one of the names in
 * `OQS_SIG_algs`.
...
typedef struct OQS_SIG {
   ...
   *
   * Implementations with the same method_name and same alg_version will be
   * interoperable. See README.md for information about algorithm compatibility.
   */
  const char *alg_version;

  /** The NIST security level (1, 2, 3, 4, 5) claimed in this algorithm's
   ...
   * `secret_key`, based on the `length_*` members in this object or the
   ...
   *
   * @param[out] signature The signature on the message represented as a byte
   * string.
   * @param[out] signature_len The actual length of the signature. May be
   * smaller than `length_signature` for some algorithms since some algorithms
   * have variable length signatures.
   ...
   * @param[in] signature The signature on the message represented as a byte
   * string.
 ...
 *
 * Callers should always check whether the return value is `NULL`, which
 * indicates either than an invalid algorithm name was provided, or that the
 * requested algorithm was disabled at compile-time.
 *
 * @param[in] method_name Name of the desired algorithm; one of the names in
 * `OQS_SIG_algs`.
 * @return An OQS_SIG for the particular algorithm, or `NULL` if the algorithm
 ...
 * @return OQS_SUCCESS or OQS_ERROR
 */
OQS_API OQS_STATUS OQS_SIG_keypair(const OQS_SIG *sig, uint8_t *public_key,
                                   uint8_t *secret_key);
